...
const BookFilter = ({ books, initialBooks, showFilter, customGridCols, customHeight }) => {
    ...
    <>
      ...
      <div
        className={`grid grid-cols-1 sm:grid-cols-2 ${
          customGridCols || "md:grid-cols-3"} gap-4 relative `}
      >
          {
          filteredBooks.map((book, index) => (
            <div
              key={book.id || index}
              className={`bookFilter shadow-md hover:shadow-2xl rounded p-4 flex flex-col items-center ${customHeight} truncate leading-4`}
            >
              <Image
